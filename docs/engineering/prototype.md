...
`prototype` 构建一个小型、用完即弃的程序，它唯一的职责就是回答一个设计问题——这个 state model 感觉对吗，或者这个 UI 应该长什么样。

这份代码**从第一天起就是 throwaway 的**，并且被明确如此标记。它不带 tests，除了能让它跑起来之外没有任何错误处理，没有抽象，也没有持久化。重点在于快速学到点东西然后把它删掉——所以一旦你开始加固它，你就已经不再是在做 prototype 了。

## When to reach for it

输入 `/prototype`，或者当任务合适时由 agent 自动取用它。
...
- **“这个 logic / state model 感觉对吗？”**——一个小型的交互式 terminal app，推动 state machine 走过那些棘手的 case，在每次动作之后打印完整 state，让你能观察什么发生了变化。
- **“这应该长什么样？”**——在同一条 route 上的几个截然不同的 UI variations，可通过一个浮动栏切换，这样你对比的是真实的渲染，而不是凭空想象。
...
prototype 不属于 main branch：没有 tests，没有错误处理，没有任何需要维护的东西。但这不是销毁它的理由。一旦答案被捕获，就把任何经过验证的 decision 折叠进真实代码，然后把 prototype 捕获到一条 throwaway branch 上——移出 main、永不合并——并在实现 issue 上留下一个指向它的 context pointer。main branch 保持干净；而那份原始探索对任何想重新运行它的人来说仍一键可达。一条烂在 main branch 里的 prototype 已经过了它的用途——一份作为 primary source 捕获在旁支 branch 上的 prototype 则没有。

## Where it fits

`prototype` 是一个可随时取用的 standalone：你进入它来解决一个设计问题，然后再退出来。它的答案往往会喂给下一步——一个经过验证的 state model 或 UI 方向会成为 [to-spec](https://aihero.dev/skills-to-spec) 要落笔写下的已定输入，或者一个值得通过 [domain-modeling](https://aihero.dev/skills-domain-modeling) 记录下来的架构 decision。当你不确定哪个 skill 或 flow 合适时，[ask-matt](https://aihero.dev/skills-ask-matt) 会为你引路。
