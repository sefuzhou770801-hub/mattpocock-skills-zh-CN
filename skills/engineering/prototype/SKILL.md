...
- **"Does this logic / state model feel right?"** → [LOGIC.md](LOGIC.md)。构建单个可分享 HTML 文件——自由点选按钮加上分页 guided walkthroughs——推动 state machine 跑过纸面上难以推理的 cases，并让非开发者也能驱动。
...
2. **启动要 trivial。** UI prototype 用项目现有 task runner 的一条命令启动——`pnpm <name>`、`python <path>`、`bun <path>` 等。Logic demo 是用户双击打开的单个 HTML 文件。无论哪种，启动都不必动脑。
