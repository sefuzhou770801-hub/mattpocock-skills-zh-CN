...
`engineering/` 或 `productivity/`（**promoted** 桶）中的每个 skill，都必须在顶层 `README.md` 中有引用，并在 `.claude-plugin/plugin.json` 的 `skills` 数组中有条目（Claude Code plugin 恰好发布 promoted 集合）。`misc/`、`in-progress/` 和 `deprecated/` 中的 skills 不得出现在这两个位置。

本仓库的公开 skill 集合同时包含 `misc/`（本 fork 的既有策略）。顶层 `README.md` 中的每个 skill 条目都必须把 skill 名称链接到对应的 `SKILL.md`。

每个 bucket folder 都有一个 `README.md`，列出该 bucket 中的所有 skills，并给出一行描述；skill 名称需要链接到对应的 `SKILL.md`。Promoted buckets 的 `README.md` 和顶层 `README.md` 都按 **User-invoked** 与 **Model-invoked** 分组；非 promoted bucket（`misc/`、`in-progress/`）用扁平列表。
